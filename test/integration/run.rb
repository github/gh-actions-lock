...
LOCKFILE_TEMPLATES = {
  ...
  # Lockfile with a branch ref (main) recorded at a stale-but-reachable SHA
  # (an ancestor of the live head). Used to exercise --relock bumping a moved
  # branch ref to the current head.
  "stale_checkout_main" => -> {
    build_lockfile(
      workflows: {
        ".github/workflows/ci.yml" => [
...
# GraphQL handler for a benign branch move: actions/checkout@main resolves to
# MAIN_BRANCH_SHA (the live head), and reachability checks report BEHIND so the
# recorded ancestor SHA is a plain ref-moved (not an unreachable pin). Used by
# the --relock scenario to bump a moved branch ref to the current head.
def checkout_graphql_relock(srv)
  srv.on(:POST, %r{/graphql$}) do |req|
    body = JSON.parse(req.body) rescue {}
    query = body["query"] || ""
    ...
    if query.include?("expression")
      [200, { "Content-Type" => "application/json" },
       JSON.generate({ data: { a0: {
         nameWithOwner: "actions/checkout",
         object: { oid: MAIN_BRANCH_SHA, file: { object: { text: "name: Checkout\ndescription: Checkout\n" } } }
       } } })]
    elsif query.include?("compare")
      repo_data = {}
      query.scan(/b(\d+):/).flatten.each { |i| repo_data["b#{i}"] = { compare: { status: "BEHIND" } } }
