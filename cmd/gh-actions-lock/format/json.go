	...
	buildFindings := func() []Finding {
		...
		for _, wr := range report.Workflows {
			for _, f := range wr.Findings {
				if f.Category == checks.RunOnly || (f.Category == checks.LocalAction && f.Severity != checks.SeverityError) || (f.Category == checks.SelfHostedRunner && f.Severity != checks.SeverityError) || (f.Category == checks.Valid && f.Severity == checks.SeverityOK) {
					continue
				}
				allFindings = append(allFindings, findingFromReport(f))
			}
