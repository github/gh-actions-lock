...
func runCheck(cmd *cobra.Command, opts *checkOptions, newResolver resolverFunc) error {
	...
	// reusing the resolver's unified API client. Cooldown is read from the
	// repo's Dependabot config (or is absent — there is no built-in default).
	var tagger *tag.Lister
	var cooldownCfg tag.CooldownConfig
	var cooldownWarnings []string
	if gc := r.GHClient(); gc != nil {
		cooldownCfg, cooldownWarnings = config.LoadCooldown(".")
	...
	// pinned tags when no cooldown is configured. Both are non-blocking.
	appendCooldownConfigFindings(report, cooldownWarnings)
	if tagger != nil && cooldownCfg.DefaultDays <= 0 {
