...
`SeverityWarning` / `ConfidenceMedium`. That is the *correct* default partition: it
keeps us from raising forgery alerts on a transient API blip. But it has a known
blind spot — **a real lockfile forgery whose ancestry check happens to be
rate-limited gets reported at warning severity**, the same level as a benign
release tag move. An attacker who can time their tampering against a Compare-API
brown-out (or simply submit during a noisy CI run with hundreds of refs) gets
demoted from "investigate immediately" to "warning."
...
For a finding currently at `CategoryAncestryUnknown / SeverityWarning`, given
the reachability state of the **lockfile-pinned SHA** (`pin.Hex`) against the
upstream ref:

| Reachability(`pin.Hex`, ref)               | Outcome                                                                   | Severity / Confidence | Rationale                                                                                                                |
| ------------------------------------------ | ------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `Reachable`                                | Keep `CategoryAncestryUnknown`; enrich detail.                            | warning / medium      | Locked SHA is in legitimate lineage. The observed SHA being newer is most plausibly a benign tag move. Forgery unlikely. |
| `Unreachable`                              | **Suppress** `CategoryAncestryUnknown`; let `CategoryImpostorCommit` win. | (n/a — suppressed)    | The pinned SHA is not on any branch — `checkImpostorCommit` already emits an impostor finding at error severity.         |
| `ReachabilityUnknown` (incl. "not asked")  | Keep `CategoryAncestryUnknown`; enrich detail to note compound uncertainty. | warning / low         | Both Compare and branch_commits failed (or branch_commits was never run for this dep). Confidence drops; severity unchanged because we still can't claim forgery. The current `checkResolver` interface conflates these two states (see prewarmedResolver behavior); see Open Questions. |
...
- **No new cross-finding category.** The existing taxonomy
  (`CategoryAncestryUnknown`, `CategoryImpostorCommit`,
  `CategoryLockfileForgery`, `CategoryRefMoved`) covers the partition. Adding
  a fifth category would just split tooling that already keys on the
  existing four.
...
| ------------------- | ---------------------------------------------- | ---------------------------------------------------------------- |
| `AncestryUnknown`   | `Reachable`                                    | `CategoryAncestryUnknown` (warning, enriched).                   |
| `AncestryUnknown`   | `Unreachable`                                  | `CategoryImpostorCommit` only (error). AncestryUnknown suppressed. |
| `AncestryUnknown`   | `ReachabilityUnknown` (incl. absent in prewarm)| `CategoryAncestryUnknown` (warning, low confidence, enriched).   |
...
1. **SARIF / terminal output**: the `CategoryAncestryUnknown` finding's
   detail string is rendered downstream in `cmd/gh-actions-pin/format/`.
   Confirm the enriched detail (with reachability summary) renders
   cleanly in both terminal and SARIF without exceeding any known
   downstream size limits.
