...
// TagLister in-memory caches (cache, repoCache, releaseDates) are keyed by
...
//
// The cache type system can't enforce the discipline (singleflight requires
// string keys), so a direct repoCache["Owner/Repo"] would compile silently
// and reintroduce the bug. This test guards against that regression by
// driving the public API with two case variants and asserting only one
// cache entry results.
//
...
func TestTagLister_CaseInsensitiveCacheKeys(t *testing.T) {
	...
	tl.mu.Lock()
	gotTags := len(tl.cache)
	gotRepo := len(tl.repoCache)
	gotDates := len(tl.releaseDates)
	tl.mu.Unlock()
