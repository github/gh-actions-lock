...
// than re-querying for every engine call); ancestry lookups and tag-object
// peels stay on-demand because they are infrequent (one per finding) and we
// want the resolver's own cache to handle dedupe.
...
func (a *engineResolver) PeelTagObject(_ context.Context, owner, repo, sha string) (string, bool) {
	if a.inner == nil {
		return "", false
	}
