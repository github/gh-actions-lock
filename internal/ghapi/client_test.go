...
// --- SSO header extraction tests ---

func TestSSOURL_ExtractsFromHeader(t *testing.T) {
	c := &Client{}
	c.ssoHeader = `required; url=https://github.com/orgs/actions/sso?authorization_request=ABCDEF`
	got := c.SSOURL()
	want := "https://github.com/orgs/actions/sso?authorization_request=ABCDEF"
	if got != want {
		t.Errorf("SSOURL() = %q, want %q", got, want)
	}
}

func TestSSOURL_EmptyWhenNoHeader(t *testing.T) {
	c := &Client{}
	if got := c.SSOURL(); got != "" {
		t.Errorf("SSOURL() = %q, want empty", got)
	}
}

func TestSSOURL_EmptyWhenMalformed(t *testing.T) {
	c := &Client{}
	c.ssoHeader = "required; no-url-here"
	if got := c.SSOURL(); got != "" {
		t.Errorf("SSOURL() = %q, want empty", got)
	}
}

func TestSSOTransport_CapturesHeader(t *testing.T) {
	c := &Client{}
	tr := &ssoTransport{
		inner: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			resp := &http.Response{
				StatusCode: 403,
				Header:     http.Header{"X-Github-Sso": {"required; url=https://github.com/orgs/test/sso?authorization_request=XYZ"}},
				Body:       io.NopCloser(bytes.NewReader(nil)),
			}
			return resp, nil
		}),
	...
	want := "https://github.com/orgs/test/sso?authorization_request=XYZ"
	if got := c.SSOURL(); got != want {
		t.Errorf("after RoundTrip, SSOURL() = %q, want %q", got, want)
	}
}
