...
// DepChange pairs an old and new dependency that represent the same logical
...
// DepDiff is the result of comparing two dependency lists.
//
// Matching strategy:
...
type DepDiff struct {
	Changed   []DepChange  // Same Key, SHA differs
	Rekeyed   []DepChange  // Same NWO, different ref (tag moved)
...
func DiffDeps(old, new []Dependency) DepDiff {
	var diff DepDiff
	...
	for _, dep := range new {
		...
		if strings.EqualFold(oldDep.SHA, dep.SHA) {
			...
			diff.Changed = append(diff.Changed, DepChange{Old: oldDep, New: dep})
		}
	}
	...
	for _, dep := range new {
		...
		handledNew[dep.Key()] = true
		diff.Rekeyed = append(diff.Rekeyed, DepChange{Old: *match, New: dep})
	}
